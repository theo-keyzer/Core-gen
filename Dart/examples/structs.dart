...
class KpComp extends Kp 
{
	...
	List get_var(glob, va, lno) {
		if (va[0].compareTo("parent") == 0) { // tst.unit:16, d_struct.act:632
			if (k_parentp >= 0 && va.length > 1) {
				return( glob.dats.ap_comp[ k_parentp ].get_var(glob, va.sublist(1), lno) );
			}
		}
		if (va[0].compareTo("Comp_parent") == 0 && va.length > 1) { // tst.unit:16, d_struct.act:727
		...
		if (va[0].compareTo("Ref_comp") == 0 && va.length > 1) { // tst.unit:54, d_struct.act:727
		...
		if (va[0].compareTo("Element") == 0 && va.length > 2) { // tst.unit:24, d_struct.act:432
		...
		if (va[0].compareTo("Ref") == 0 && va.length > 2 && itsref.length > 0) { // tst.unit:39, d_struct.act:444
	...
	int do_its(glob, va, lno) {
		if (va[0].compareTo("Element") == 0) { // tst.unit:19, d_struct.act:706
		...
		if (va[0].compareTo("Ref") == 0) { // tst.unit:39, d_struct.act:706
		...
		if (va[0].compareTo("Comp_parent") == 0) { // tst.unit:16, d_struct.act:537
		...
		if (va[0].compareTo("Ref_comp") == 0) { // tst.unit:54, d_struct.act:537
		...
		if ( va[0].compareTo("Child") == 0 ) { // tst.unit:2, d_struct.act:156
...
class KpElement extends Kp 
{
	...
	bool load(act, ln, pos, lno) {
		...
		p, names["doc"] = getws(ln, p);
		var parentp = act.ap_comp.size-1;
		...
		var s = parentp.toString() + "_Element_" + names["name"];
		act.index[s] = me;
		return true;
	}
	...
	List get_var(glob, va, lno) {
		...
		if (va[0].compareTo("Ref_element") == 0 && va.length > 1) { // tst.unit:53, d_struct.act:727
			for (var st in glob.dats.ap_ref) {
				if (st.k_elementp == me) {
					return (st.get_var(glob, va.sublist(1), lno) );
				}
	...
	int do_its(glob, va, lno) {
		if (va[0].compareTo("parent") == 0) { // tst.unit:2, d_struct.act:512
			if (parentp >= 0) {
		...
		if (va[0].compareTo("Ref_element") == 0) { // tst.unit:53, d_struct.act:537
			for (KpRef st in glob.dats.ap_ref) {
				if (st.k_elementp == me) {
					if (va.length > 1) {
						var ret = st.do_its(glob, va.sublist(1), lno);
		...
		if ( va[0].compareTo("Child") == 0 ) { // tst.unit:19, d_struct.act:156
			return(0);
		}
...
class KpRef extends Kp 
{
	...
	List get_var(glob, va, lno) {
		if (va[0].compareTo("element") == 0) { // tst.unit:53, d_struct.act:632
			if (k_elementp >= 0 && va.length > 1) {
				return( glob.dats.ap_element[ k_elementp ].get_var(glob, va.sublist(1), lno) );
			}
		}
		if (va[0].compareTo("comp") == 0) { // tst.unit:54, d_struct.act:632
	...
	int do_its(glob, va, lno) {
		...
		if ( va[0].compareTo("Child") == 0 ) { // tst.unit:39, d_struct.act:156
			return(0);
		}
...
class KpC extends Kp 
{
	...
	bool load(act, ln, pos, lno) {
		...
		me = act.ap_c.length;
		p, k_desc = getws(ln, p);
		var parentp = act.ap_actor.size-1;
		if (parentp < 0 ) { 
